...
/* Private variables ---------------------------------------------------------*/
/* USER CODE BEGIN PV */
UX_SLAVE_CLASS_CDC_ACM  *cdc_acm1, *cdc_acm2;
...
VOID USBD_CDC_ACM_Activate1(VOID *cdc_acm_instance)
{
  ...
  if (ux_device_class_cdc_acm_ioctl(cdc_acm1, UX_SLAVE_CLASS_CDC_ACM_IOCTL_SET_LINE_CODING,
...
VOID USBD_CDC_ACM_ParameterChange1(VOID *cdc_acm_instance)
{
  ...
  switch (request)
  {
    case UX_SLAVE_CLASS_CDC_ACM_SET_LINE_CODING :
      ...
      if (ux_device_class_cdc_acm_ioctl(cdc_acm1, UX_SLAVE_CLASS_CDC_ACM_IOCTL_GET_LINE_CODING,
                                        &CDC_VCP_LineCoding) != UX_SUCCESS)
      {
        Error_Handler();
      }
...
VOID usbx_cdc_acm_read_thread_entry(ULONG thread_input)
{
  ...
  while (1)
  {
    ...
    if ((device->ux_slave_device_state == UX_DEVICE_CONFIGURED) && (cdc_acm1 != UX_NULL))
    {
      ...
      ux_device_class_cdc_acm_read(cdc_acm1, (UCHAR *)UserRxBufferFS, 64,
                                   &actual_length);
      if (actual_length != 0)
      {
...
VOID usbx_cdc_acm_write_thread_entry(ULONG thread_input)
{
  ...
  while (1)
  {
    ...
    if (UserTxBufPtrOut != UserTxBufPtrIn)
    {
      ...
      if (ux_device_class_cdc_acm_write(cdc_acm1, (UCHAR *)(&UserTxBufferFS[buffptr]),
